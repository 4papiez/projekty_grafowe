...
//------------------------------------------------
void Graf::generujGrafKraw(const int n,const int l,const int maxi_wart){
    ...
    if(grafRzadki==false){
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++)
                _tab[i][j]=(i!=j)?1+std::rand()%maxi_wart:0;
        }
    }
    ...
    for(int i=0;i<tl;i++){
        ...
        _tab[n2][n1]=_tab[n1][n2]=1+std::rand()%maxi_wart;

    }
    init_random=false;
}
void Graf::generujGrafPrawd(const int n,const double p,const int maxi_wart){
	if(p<0||p>1)
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            if(((double)(std::rand()%1000))/1000<p && i!=j){
                _tab[j][i]=_tab[i][j]=1+std::rand()%maxi_wart;
				_l++;
            }
		}
	}
...
void GrafBase::zapiszGraf(const std::string str,int** tab,const int n,const int l){
  std::fstream plik;
